// Package transaction defines the validation boundary consumed by the ABCI app.
//
// Consensus and EVM execution depend on Validator, not on one wire format or
// signature scheme. An ECDSA transaction, a hybrid ECDSA+ML-DSA
...
// NewValidationError lets pluggable transaction modules report failures using
// the stable categories understood by the node application.

...
import "github.com/huyCuong73/pluto/modules/transaction"
...
func (v *OptInHybridValidator) Validate(raw []byte) (*transaction.ValidatedTransaction, error) {
	if v == nil {
		return nil, transaction.NewValidationError(transaction.FailurePQC, "opt-in hybrid validator is nil")
	}
	...
	if ethereumErr == nil {
		...
		if err != nil {
			return nil, transaction.NewValidationError(transaction.FailurePQC, "resolve post-quantum policy for %s: %w", validated.Sender.Hex(), err)
		}
		if registered {
			return nil, transaction.NewValidationError(transaction.FailurePQC, "%w: %s", ErrPQRequiredForSender, validated.Sender.Hex())
		}

...
type hybridFixture struct {
	...
	envelopeRaw  []byte
	scheme       pqc.MLDSA65
	ecdsa        *transaction.ECDSAValidator
}
...
func newHybridFixture(t testing.TB) hybridFixture {
	...
	ecdsa, err := transaction.NewECDSAValidator(hybridTestChainID)
	if err != nil {
...
func TestHybridValidatorRequiresECDSAKeyBindingAndMLDSA65(t *testing.T) {
	...
	t.Run("malformed envelope", func(t *testing.T) {
		...
		if transaction.FailureOf(err) != transaction.FailureEncoding {
			t.Fatalf("failure = %v (%v), want encoding", transaction.FailureOf(err), err)
		}
	})

	t.Run("unregistered sender", func(t *testing.T) {
		_, err := fixture.validator(t, PQKeyHash{}, false).Validate(fixture.envelopeRaw)
		if transaction.FailureOf(err) != transaction.FailurePQC || !errors.Is(err, ErrPQKeyNotRegistered) {
	...
	t.Run("wrong binding", func(t *testing.T) {
		...
		if transaction.FailureOf(err) != transaction.FailurePQC || !errors.Is(err, ErrPQKeyBindingMismatch) {
	...
	t.Run("modified ML-DSA signature", func(t *testing.T) {
		...
		if transaction.FailureOf(err) != transaction.FailurePQC || !errors.Is(err, pqc.ErrVerificationFailed) {
	...
	t.Run("wrong EVM chain ID", func(t *testing.T) {
		...
		if transaction.FailureOf(err) != transaction.FailureAuthentication {
			t.Fatalf("failure = %v (%v), want authentication", transaction.FailureOf(err), err)
		}
	})
}
...
func TestNewHybridValidatorRejectsMissingDependencies(t *testing.T) {
	...
	tests := []struct {
		name     string
		chainID  int64
		ethereum transaction.TransactionValidator
		scheme   pqc.Scheme

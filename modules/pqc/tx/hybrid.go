...
type HybridValidator struct {
	chainID           *big.Int
	ethereumValidator transaction.TransactionValidator
	pqScheme          pqc.Scheme
...
func (v *HybridValidator) Validate(raw []byte) (*transaction.ValidatedTransaction, error) {
	if v == nil {
		return nil, transaction.NewValidationError(transaction.FailurePQC, "hybrid validator is nil")
	}

	envelope, err := DecodeHybridEnvelopeV1(raw)
	if err != nil {
		return nil, transaction.NewValidationError(transaction.FailureEncoding, "decode hybrid transaction: %w", err)
	}
	...
	if validated == nil || validated.Message == nil {
		return nil, transaction.NewValidationError(transaction.FailureAuthentication, "Ethereum validator returned an empty transaction")
	}

	expectedHash, found, err := v.keyResolver.ExpectedKeyHash(validated.Sender)
	if err != nil {
		return nil, transaction.NewValidationError(transaction.FailurePQC, "resolve post-quantum key for %s: %w", validated.Sender.Hex(), err)
	}
	if !found {
		return nil, transaction.NewValidationError(transaction.FailurePQC, "%w: %s", ErrPQKeyNotRegistered, validated.Sender.Hex())
	}
	actualHash := HashPQPublicKey(envelope.PQPublicKey)
	if subtle.ConstantTimeCompare(actualHash[:], expectedHash[:]) != 1 {
		return nil, transaction.NewValidationError(transaction.FailurePQC, "%w: %s", ErrPQKeyBindingMismatch, validated.Sender.Hex())
	}

	signBytes, err := BuildHybridSignBytesV1(envelope, v.chainID)
	if err != nil {
		return nil, transaction.NewValidationError(transaction.FailureEncoding, "build hybrid sign bytes: %w", err)
	}
	if err := v.pqScheme.Verify(
		...
		return nil, transaction.NewValidationError(transaction.FailurePQC, "verify ML-DSA-65 transaction signature: %w", err)
	}
